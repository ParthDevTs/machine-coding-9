...
function PlayListDropDown({ videoId }) {
    ...
    return (
        <div className="relative">
            <button onClick={() => setShowDrpDown(!showDrpDwn)} className="flex items-center justify-end tracking-wider border-transparent active:border-slate-500 duration-300 active:text-slate-500">
            ...
            <ul className="w-full flex flex-col gap-2 text-sm items-start">
                {playLists.map((playlist) => {
                    return <li onClick={() => handleAddToPlaylist(videoId, playlist.name)} key={playlist._id} className="flex items-center justify-center w-full">
                        <p>{playlist.name}</p>
